...
class BaseTextCategorizationDataset:
    ...
    def get_index_to_label_map(self):
        ...
        """
        # TODO: CODE HERE
        label_map = {index: label for index, label in enumerate(self._get_label_list())}
        return label_map
    ...
    def get_label_to_index_map(self):
        ...
        """
        # TODO: CODE HERE
        index_map = {label: index for index, label in enumerate(self._get_label_list())}
        return index_map
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        # TODO: CODE HERE
        assert all(df.columns == ["post_id", "tag_name", "tag_id", "tag_position", "title"])
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                tag_occurences = df.groupby("tag_name")["post_id"].count()
                tags_over_x = tag_occurences[tag_occurences >= x]
                df = df[df["tag_name"].isin(tags_over_x.index)]
                return df
            return filter_function
