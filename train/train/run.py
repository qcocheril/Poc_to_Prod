...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    train_history = model.fit(dataset.get_train_sequence(), epochs=train_conf["epochs"], batch_size=train_conf["batch_size"], verbose=train_conf["verbose"])
    ...
    if not os.path.exists(artefacts_path):
        ...
        print(f"Directory artefacts_path created successfully.")
    ...
    if not os.path.exists(params_path):
        ...
        print(f"Train conf saved to: {params_path}")
    else:
        print(f"File already exists: {params_path}.")
    ...
    if not os.path.exists(labels_index_path):
        with open(labels_index_path, 'w') as json_file:
            json.dump(dataset.get_index_to_label_map(), json_file, indent=4)
